...
parser.add_argument('--path', default='/home/user/桌面/ImageGCN/ChestX-ray14', type=str, help='data path')
...
parser.add_argument('-e','--encoder', default='alex', type=str, help='the encoder')#res50 alex  vgg16bn
parser.add_argument('-r','--relations', default='all', type=str, help='the considered relations, pid, age, gender, view')
parser.add_argument('--pps', default='partly', type=str, help='the parameter sharing method (default partly)')#totally
...
# validate roc [0.77238562 0.90488654 0.85270768 0.67748969 0.79849249 0.7177778
#  0.68334078 0.87763223 0.7703724  0.88428628 0.86774196 0.77343687
#  0.76817673 0.87380172]
# validate average roc 0.8016091995592645
...
folder = '/home/user/桌面/ImageGCN'

model = SingleLayerImageGCN(relations, encoder=enc, inchannel=inchannel, share_encoder=pps).cuda()

# model =  nn.DataParallel(model)
# model = ImageGCN(relations, encoder=enc, inchannel=inchannel, share_encoder=pps).cuda()
...
def train(train_loader, model, criterion, optimizer, epoch, iter_size=10):
    ...
    for i, data in enumerate(train_loader):
        ...
        adj_mats2 = {key: sparse_to_tensor(value).to_dense().cuda() for key, value in adj.items()}
        data_time.update(time.time() - end)

        # from thop import profile
        # flops, params = profile(model, (inputs,  adj_mats2,  k))
        # print(flops, params)
        # aa

        output = model(inputs,  adj_mats1,  k=k)
...
if use=='train':
    ...
    cp = torch.load(join(folder,'models/%s/best_%s_%s_%s_tr%s_%s_norm.pth.tar'%(args.relations,enc,neib,mode,tr_pct,pps)))
    model.load_state_dict(cp['state_dict'])
    loss_test, avgroc_test, roc_test, pred = validate(test_loader, model, criterion)
